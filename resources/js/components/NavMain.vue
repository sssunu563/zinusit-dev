...
const findActiveGroupTitle = () =>
    props.items.find(
        (item) => hasChildren(item) && item.children?.some((child) => isCurrentOrParentUrl(child.href)),
    )?.title ?? null;
...
const isSubItemActive = (item: NavItem) => isCurrentOrParentUrl(item.href);
const isGroupExpanded = (item: NavItem) => hasChildren(item) && expandedGroupTitle.value === item.title;

const setItemElement = (title: string, element: Element | null) => {
    if (element instanceof HTMLElement) { itemElements.set(title, element); return; }
    itemElements.delete(title);
};

const cancelFlyoutClose = () => {
    if (!closeFlyoutTimer) return;
    clearTimeout(closeFlyoutTimer); closeFlyoutTimer = null;
};
const closeFlyout = () => { cancelFlyoutClose(); openFlyout.value = null; };
...
const showFlyout = (item: NavItem) => { cancelFlyoutClose(); updateFlyoutPosition(item.title); openFlyout.value = item; };
const scheduleFlyoutClose = (title: string) => {
    cancelFlyoutClose();
    closeFlyoutTimer = setTimeout(() => { if (openFlyout.value?.title === title) openFlyout.value = null; }, 120);
};
const handleParentClick = (item: NavItem, event: MouseEvent) => {
    event.preventDefault();
    if (state.value === 'collapsed') { showFlyout(item); return; }
    closeFlyout();
    expandedGroupTitle.value = expandedGroupTitle.value === item.title ? null : item.title;
};
...
watch(currentUrl, () => { expandedGroupTitle.value = findActiveGroupTitle(); }, { immediate: true });
watch(state, (s) => { if (s !== 'collapsed') closeFlyout(); });
...
<template>
        ...
        <SidebarMenu class="gap-0.5 px-1">
            <SidebarMenuItem v-for="item in props.items" :key="item.title">
                ...
                <div
                    ...
                    :ref="(el) => setItemElement(item.title, el as Element | null)"
                    class="relative"
                    @mouseenter="state === 'collapsed' ? showFlyout(item) : undefined"
                    @mouseleave="state === 'collapsed' ? scheduleFlyoutClose(item.title) : undefined"
                >
                    <SidebarMenuButton
                        :class="[
                            ...
                            isItemActive(item)
                                ...
                                : 'border-transparent text-muted-foreground hover:bg-primary/5 hover:text-primary transition-colors'
                        ]"
                    ...
                    >
                        ...
                        <SidebarMenuBadge
                            ...
                            :class="isItemActive(item) ? 'bg-primary text-white' : 'bg-muted text-muted-foreground'"
                        >
                            {{ item.badge }}
                        </SidebarMenuBadge>
                        <ChevronDown
                ...
                <Transition name="app-collapse">
                    ...
                    >
                        <SidebarMenuSub class="mt-0.5 ml-3 space-y-0.5 border-l border-border pl-3">
                            <SidebarMenuSubItem
                                v-for="child in item.children"
                                :key="`${item.title}-${child.title}`"
                            >
                                <SidebarMenuSubButton class="h-8 rounded-md" as-child :is-active="isSubItemActive(child)">
                                    <Link
                                        :href="child.href"
                                        :class="[
    ...
    <teleport to="body">
        <div
            v-if="state === 'collapsed' && openFlyout && openFlyout.children?.length"
        ...
        >
            ...
            <div class="space-y-0.5">
                <Link
                    ...
                    @click="closeFlyout()"
                >
                    <component v-if="child.icon" :is="child.icon" class="size-3.5 shrink-0 text-muted-foreground" />
                    <span class="truncate">{{ child.title }}</span>
                </Link>
            </div>
        </div>
