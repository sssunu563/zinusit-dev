            ...
            <div class="mb-8 flex flex-col lg:flex-row lg:items-center justify-between gap-4">
                <div class="relative w-full lg:max-w-md">
                    ...
                    <input
                        ...
                        type="text"
                        placeholder="Cari nomor dokumen, catatan, atau otorisator..."
                ...
                <div class="flex items-center gap-2">
                    <!-- Form Filter Quick Selector -->
                    <select
                        v-model="filterForm.filter_form"
                        ...
                        >
                            ...
                            >
                                <div class="flex items-center justify-between mb-6">
                                    <h3 class="text-[10px] font-black uppercase tracking-widest text-slate-400">Filter Log Formulir</h3>
                                    <button 
                                        @click="filterForm.search = ''; filterForm.filter_form = ''; filterForm.filter_action = ''; filterForm.filter_admin = ''; clearDateFilters(); showFilters = false;"
                ...
                <table class="w-full border-collapse">
                    ...
                    <tbody class="divide-y divide-slate-50">
                        <tr v-for="log in logs.data" :key="log.id" class="group hover:bg-slate-50/50 transition-colors">
                            ...
                            <!-- Form Type & Doc Number -->
                            <td class="px-6 py-4">
                                <div class="flex items-center gap-2">
                                    <div class="h-7 w-7 rounded-lg bg-emerald-50 text-[#003628] flex items-center justify-center shrink-0">
                                        <component :is="getFormIcon(log.form_type)" class="size-3.5" />
                                    </div>
                                    <div class="space-y-0.5">
                                        <div class="flex items-center gap-1.5">
                                            <span class="text-[10px] font-black uppercase tracking-wider text-slate-400">{{ log.form_name }}</span>
                                        </div>
                                        <Link
                                            v-if="log.doc_url"
                                            :href="log.doc_url"
                                        ...
                                        >
                                            <span>{{ log.doc_no || 'Buka Dokumen' }}</span>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex flex-col gap-1 items-start">
                                    ...
                                    <span v-if="log.role" class="text-[10px] font-bold text-slate-400">
                                        Role: {{ log.role }}
                                    </span>
                                </div>
                            </td>
                        ...
                        <tr v-if="logs.data.length === 0">
                            <td colspan="6" class="px-6 py-20 text-center text-slate-400 italic text-sm">
                                Tidak ada catatan log formulir yang sesuai dengan kriteria pencarian / filter Anda.
                            </td>
                        </tr>
                    </tbody>
                </table>
            ...
            <div class="md:hidden space-y-3">
                ...
                >
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-2">
                            <component :is="getFormIcon(log.form_type)" class="size-4 text-[#003628]" />
                            <span class="text-xs font-black text-slate-900">{{ log.form_name }}</span>
                        </div>
                        <span
                    ...
                    <div class="space-y-1">
                        <p v-if="log.doc_no" class="text-xs font-mono font-bold text-[#003628]">{{ log.doc_no }}</p>
