...
class SetupServerOperationComponents extends Command
{
    protected $signature = 'setup:server-operation';
    ...
    public function handle()
    {
        ...
        if (!File::isDirectory($baseDir)) {
            ...
            $this->info("✓ Created directory: {$baseDir}");
        }
...
import {
    Server, Activity, Thermometer, Database,
    LayoutDashboard, Filter, FileDown, Play, RefreshCw, Loader2,
        ...
        <div class="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 p-6">
            ...
            <div class="mb-8">
                <div class="flex items-center gap-3 mb-3">
                    <div class="p-3 bg-blue-100 rounded-lg">
                        <Server class="w-7 h-7 text-blue-600" />
                    </div>
                    <div>
            ...
            <div class="bg-white rounded-lg shadow mb-6">
                ...
                <div class="flex gap-2 p-4 border-b border-slate-200 flex-wrap">
                    <button
                        @click="openFlyout = openFlyout === 'filter' ? null : 'filter'"
                        class="flex items-center gap-2 px-4 py-2 bg-slate-100 text-slate-700 rounded-lg hover:bg-slate-200 transition"
                    >
                    ...
                    <button
                        @click="openFlyout = openFlyout === 'fetch' ? null : 'fetch'"
                ...
                <div v-if="openFlyout === 'filter'" class="p-4 bg-slate-50 border-b border-slate-200">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input v-model="filterFrom" type="date" class="w-full px-3 py-2 border border-slate-300 rounded-lg" />
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-slate-700 mb-1">To Date</label>
                            <input v-model="filterTo" type="date" class="w-full px-3 py-2 border border-slate-300 rounded-lg" />
                        </div>
                    </div>
                    <button
                        @click="closeFlyout"
                ...
                <div v-if="openFlyout === 'fetch'" class="p-4 bg-slate-50 border-b border-slate-200">
                    ...
                    <div class="mb-3">
                        <label class="block text-sm font-medium text-slate-700 mb-1">Fetch Date</label>
                        <input v-model="fetchDate" type="date" class="w-full px-3 py-2 border border-slate-300 rounded-lg" />
                    </div>
                    <button
                        @click="handleFetch"
                        :disabled="isFetching"
                        class="w-full px-4 py-2 bg-orange-600 text-white rounded-lg hover:bg-orange-700 transition disabled:opacity-50"
                    >
                        <span v-if="isFetching" class="flex items-center justify-center gap-2">
                            <Loader2 class="w-4 h-4 animate-spin" />
                            Fetching...
                        </span>
                        <span v-else>Start Fetch</span>
                    </button>
                    <div v-if="fetchMessage" class="mt-3 p-3 bg-green-100 text-green-700 rounded-lg flex items-center gap-2">
                        <CheckCircle2 class="w-5 h-5" />
                        {{ fetchMessage }}
                    </div>
                    <div v-if="fetchError" class="mt-3 p-3 bg-red-100 text-red-700 rounded-lg flex items-center gap-2">
...
<template>
    <div class="space-y-6">
        ...
        <div v-if="error" class="p-4 bg-red-100 text-red-700 rounded-lg flex items-center gap-2">
        ...
        <div v-if="summary && !loading" class="grid grid-cols-1 md:grid-cols-4 gap-4">
            <div class="bg-white p-6 rounded-lg shadow">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-slate-600 text-sm">Total Devices</p>
                        <p class="text-3xl font-bold text-slate-900 mt-1">{{ summary.total_devices }}</p>
                    </div>
                    <Server class="w-12 h-12 text-blue-500 opacity-20" />
                </div>
            </div>
            ...
            <div class="bg-white p-6 rounded-lg shadow">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-slate-600 text-sm">Avg CPU Usage</p>
                        <p class="text-3xl font-bold text-slate-900 mt-1">{{ summary.avg_cpu_usage?.toFixed(1) }}%</p>
                    </div>
            ...
            <div class="bg-white p-6 rounded-lg shadow">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-slate-600 text-sm">Avg Memory Usage</p>
                        <p class="text-3xl font-bold text-slate-900 mt-1">{{ summary.avg_memory_usage?.toFixed(1) }}%</p>
                    </div>
                    <TrendingUp class="w-12 h-12 text-orange-500 opacity-20" />
                </div>
            </div>
            ...
            <div class="bg-white p-6 rounded-lg shadow">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-slate-600 text-sm">Avg Temperature</p>
                        <p class="text-3xl font-bold text-slate-900 mt-1">{{ summary.avg_temperature?.toFixed(1) }}°C</p>
                    </div>
                    <Thermometer class="w-12 h-12 text-red-500 opacity-20" />
                </div>
            </div>
        </div>
    </div>
...
<template>
    <div>
        ...
        <div v-if="!loading && data.length > 0" class="bg-white rounded-lg shadow overflow-x-auto">
            <table class="w-full">
                <thead class="bg-slate-100 border-b border-slate-300">
                    <tr>
                        <th class="px-6 py-3 text-left text-sm font-medium text-slate-700 cursor-pointer" @click="sortBy = 'device_name'">
                            <div class="flex items-center gap-2">
                                Device <component :is="sortBy === 'device_name' ? (sortDesc ? ChevronDown : ChevronUp) : Minus" class="w-4 h-4" />
                            </div>
                        </th>
                        ...
                        <th class="px-6 py-3 text-left text-sm font-medium text-slate-700 cursor-pointer" @click="sortBy = 'cpu_usage_percent'">
                            <div class="flex items-center gap-2">
                                CPU (%) <component :is="sortBy === 'cpu_usage_percent' ? (sortDesc ? ChevronDown : ChevronUp) : Minus" class="w-4 h-4" />
                            </div>
                        </th>
                ...
                <tbody class="divide-y divide-slate-200">
                    <tr v-for="device in sorted" :key="device.host_id" class="hover:bg-slate-50">
                        ...
                        <td class="px-6 py-3 text-sm">
                            <div class="flex items-center gap-2">
                                <div class="w-16 bg-slate-200 rounded h-2">
                                    <div class="bg-blue-500 rounded h-2" :style="{ width: (device.cpu_usage_percent || 0) + '%' }"></div>
                                </div>
                                <span class="text-slate-700">{{ device.cpu_usage_percent?.toFixed(1) }}%</span>
                            </div>
                        </td>
                        <td class="px-6 py-3 text-sm">
                            <div class="flex items-center gap-2">
                                <div class="w-16 bg-slate-200 rounded h-2">
                                    <div class="bg-green-500 rounded h-2" :style="{ width: (device.memory_usage_percent || 0) + '%' }"></div>
                                </div>
                                <span class="text-slate-700">{{ device.memory_usage_percent?.toFixed(1) }}%</span>
                            </div>
                        </td>
        ...
        <div v-if="!loading && data.length === 0" class="text-center py-8 text-slate-600">
            ...
            <table class="w-full">
                <thead class="bg-slate-100 border-b border-slate-300">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-sm font-medium text-slate-700 cursor-pointer" @click="sortBy = 'location'">
                            <div class="flex items-center gap-2">
                                Location <component :is="sortBy === 'location' ? (sortDesc ? ChevronDown : ChevronUp) : Minus" class="w-4 h-4" />
                            </div>
                        </th>
                        <th class="px-6 py-3 text-left text-sm font-medium text-slate-700">Description</th>
                        <th class="px-6 py-3 text-left text-sm font-medium text-slate-700 cursor-pointer" @click="sortBy = 'value_celsius'">
                            <div class="flex items-center gap-2">
                                Temperature <component :is="sortBy === 'value_celsius' ? (sortDesc ? ChevronDown : ChevronUp) : Minus" class="w-4 h-4" />
                            </div>
                        </th>
                ...
                <tbody class="divide-y divide-slate-200">
                    <tr v-for="sensor in sorted" :key="sensor.id" class="hover:bg-slate-50">
                        ...
                        <td class="px-6 py-3 text-sm">
                            <span :class="['px-3 py-1 rounded-full font-medium flex items-center gap-2 w-fit', getTempColor(sensor.value_celsius)]">
                                <Thermometer class="w-4 h-4" />
                                {{ sensor.value_celsius?.toFixed(1) }}°C
                            </span>
                        </td>
...
VUE;
        ...
        $files = [
            'Index.vue' => $indexVue,
            'TabSummary.vue' => $tabSummary,
            'TabServerData.vue' => $tabServerData,
            'TabTemperature.vue' => $tabTemperature,
        ];
        ...
        $this->info("\n✅ Setup complete! All Vue components created.");
        $this->line("Now run: npm run build");
    }
}
VUE;
